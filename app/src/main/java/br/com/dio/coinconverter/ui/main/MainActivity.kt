...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreateOptionsMenu(menu: Menu?): Boolean {
        ...
        menu?.findItem(R.id.action_converter)?.isVisible = false
    ...
    private fun bindListeners() {
        ...
        binding.icSwitch.setOnClickListener {
            val tvFromText = binding.tvFrom.text
            binding.tvFrom.setText(binding.tvTo.text, false)
            binding.tvTo.setText(tvFromText, false)
        }
        ...
        binding.btnSave.setOnClickListener {
            ...
            (value as? MainViewModel.State.Success)?.let {
                val date = getCurrentDateTime().toString("dd/MM/yyyy HH:mm")
                val splitDateTime = date.split(" ").toTypedArray()
                val exchange = it.exchange.copy(
                    bid = it.exchange.bid * binding.tilValue.text.toDouble(),
                    inputValue = binding.tilValue.text.toDouble(),
